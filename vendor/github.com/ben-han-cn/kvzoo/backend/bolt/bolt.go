...
func (db *BoltDB) Tables() ([]string, error) {
	tx, err := db.db.Begin(false)
	if err != nil {
		return nil, err
	}
...
func tables(c *bolt.Cursor, isTopLevel bool) ([]string, error) {
	k, v := c.First()
	if k == nil {
		return nil, nil
	}
	...
	if v != nil {
		if isTopLevel {
			return nil, fmt.Errorf("broken db which has top level key:%v, value:%v", string(k), v)
		}
	} else {
	...
	for {
		...
		if v != nil {
			if isTopLevel {
				return nil, fmt.Errorf("broken db which has top level key:%s, value:%v", string(k), v)
			}
		} else {
			tables = append(tables, string(k))
		}
...
func (tx *TableTX) Tables() ([]string, error) {
	return tables(tx.bucket.Cursor(), false)
}
